 ...
 * @return  void.
 */
void EBFPIDDebuger_Data_Receive(uint8_t *data, uint16_t data_len)
{
 ...
 * @return  初始化结果.
 */
int32_t EBFPIDDebuger_init(void)
{
 ...
 */
int8_t EBFPIDDebuger_Receiving_Process(EBFPIDDebuger_Opts *opts)
{
    ...
    while (1)
    {
        ...
        case SET_P_I_D_CMD:
        {
            ...
            opts->SetPID(p_temp, i_temp, d_temp); // 设置 P I D
        }
        break;
        ...
        case SET_PERIOD_CMD:
        {
            ...
            if (opts->Set_Period != NULL)
            {
                opts->Set_Period(temp); // 设置定时器周期1~1000ms
            }
        }
        break;
 ...
 * @retval 无
 */
void EBFPIDDebuger_Set_Computer_Value(EBFPIDDebuger_Opts *opts, uint8_t cmd, uint8_t ch, void *data, uint8_t num)
{
    uint8_t sum = 0; // 校验和
    num *= 4;        // 一个参数 4 个字节
    ...
    if (opts->Transmit != NULL)
    {
        opts->Transmit((uint8_t *)&set_packet, sizeof(set_packet)); // 发送数据头
        opts->Transmit((uint8_t *)data, num);                       // 发送参数
        opts->Transmit((uint8_t *)&sum, sizeof(sum));               // 发送校验和
    }
}

int32_t EBFPIDDebuger_Get_Target_Value(void)
{
    return set_point;
}
